...
describe Commands::Place do
  let(:command) do
    described_class.new(simulation: simulation, arguments: arguments)
  end

  let(:simulation) do
    instance_double(Simulation, table_position_available?: true)
  end
  ...
  describe '#valid?' do
    ...
    context 'with invalid table position' do
      let(:simulation) do
        instance_double(Simulation, table_position_available?: false)
      end
